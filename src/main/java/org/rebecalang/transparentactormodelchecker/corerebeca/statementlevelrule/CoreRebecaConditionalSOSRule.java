...
@Component
public class CoreRebecaConditionalSOSRule extends AbstractSOSRule<Pair<CoreRebecaActorState, InstructionBean>> {
	...
	public CoreRebecaDeterministicTransition<Pair<CoreRebecaActorState, InstructionBean>> applyRule(
		...
		JumpIfNotInstructionBean jinsb = (JumpIfNotInstructionBean) source.getSecond();

		Object conditionValue = false;
		if(jinsb.getCondition() != null)
			conditionValue = CoreRebecaAssignmentSOSRule.getValue(
					jinsb.getCondition(), source.getFirst());

		if((Boolean)conditionValue) {
			source.getFirst().movePCtoTheNextInstruction();
		} else {
			source.getFirst().setPC(new Pair<String, Integer>(jinsb.getMethodName(), jinsb.getLineNumber()));						
		}
